...
- 저장 전 정규화: name 공백 제거(`"홍 길 동"` → `"홍길동"`, 최대 10자), phone 숫자만 추출(`"010-1234-5678"` → `"01012345678"`, 최대 11자), address 최대 30자

**정원 초과 방지 (동시성 대응)**
- 트랜잭션 + Row Lock: Course 행 잠금 → applied 수 확인 → 초과 시 실패 → 정원 내 시 Registration 생성
- 정원 카운트 기준: `status = 'applied'`만 (canceled, refunded 제외)
- Rails 8은 SQLite에서 WAL 모드 + IMMEDIATE 트랜잭션이 기본 적용되어 1,000명 규모의 동시성 처리에 충분

**마감 후 신청 차단**
- 다음 중 하나라도 해당하면 신청 불가 (OR 조건)
  - 신청 마감일 경과
  - 코스 정원 초과 (applied 수 ≥ capacity)
