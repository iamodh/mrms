...
1,000명 규모 마라톤 신청 시스템의 데이터베이스로 PostgreSQL 대신 SQLite를 선택했다.
...
Rails 8은 읽기/쓰기 블로킹 최소화(WAL)와 트랜잭션 충돌 방지(IMMEDIATE)가 프레임워크 레벨에서 설정된다. `course.lock!` 같은 코드가 PostgreSQL과 다르게 동작하지만(row lock → DB lock), 이 규모에서는 차이가 체감되지 않는다.
...
# Kamal vs Vercel 배포 플랫폼 비교

| 항목 | Kamal | Vercel |
|------|-------|--------|
| **월 비용** | VPS 고정 (CX23 ~€5) | 트래픽/함수 호출 비례, 규모 커지면 급증 |
| **배포 속도** | Docker 빌드+푸시+풀 (2~5분) | git push → 수십 초 |
| **배포 편의성** | `kamal deploy` 직접 실행, 파이프라인 직접 구성 | git push만 하면 프리뷰+프로덕션 자동 |
| **스케일링** | 수동 (VPS 스펙 업그레이드) | 자동 스케일 아웃 |
| **서버 관리** | 보안패치, 모니터링, 장애 대응 직접 | 전부 위임 |
| **인프라 소유** | 내 서버, 데이터 완전 통제 | 플랫폼 종속, 정책/가격 변경 리스크 |
| **프레임워크** | Rails, Django 등 서버 앱에 최적 | Next.js에 최적, 그 외는 제약 있음 |
| **다운타임 배포** | 설정 필요 (zero-downtime 가능하나 수동) | 아토믹 배포 기본 |
| **부가 기능** | 별도 도구 설치 (모니터링, 분석 등) | Analytics, Edge Functions 등 내장 |
| **학습 가치** | Docker, 리눅스, 네트워크 전반 학습 | 플랫폼 사용법만 학습 |
| **적합한 상황** | 비용 통제 + 인프라 학습 + 서버 앱 | 빠른 출시 + 프론트엔드 중심 + 운영 최소화 |
